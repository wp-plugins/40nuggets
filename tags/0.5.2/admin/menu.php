...
function FortyNuggets_create_menu_page() {
	global $menu;
    //$menu[] = array('My Menu', 'manage_options', 'http://www.google.com', '40Nuggets', 'open-if-no-js menu-top', '40Nuggets', 'div' );
	add_menu_page(
...
//link sub-menus
add_action('admin_menu' ,'FortyNuggets_create_submenu_exteranl_link' );
function FortyNuggets_create_submenu_exteranl_link() {
	//sub-menus
	global $submenu;
	$submenu['40Nuggets'][0] = array( 'Add New', 'manage_options' , getURL("ctaBuilder"));
	$submenu['40Nuggets'][1] = array( 'Manage Nuggets', 'manage_options' , getURL("callsToAction"));
...
function getURL($target){
	...
	return "https://40nuggets.com/dashboard/{$page}.php?alk={$options->akl}";
}
